...
def execute_input(filename: str) -> int:
    ...
    for key, value in sneak.items():
        high_minute = max(value)
        if high_minute > highest_minute_value:
            highest_minute_value = high_minute
            highest_id = key
            highest_minute = value.index(max(value))

    print("%s: V%s M%s" % (highest_id, highest_minute_value, highest_minute))
